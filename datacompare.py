...
def getDifferences(i,dataframe1,dataframe2,primary_key,ind=None):
    ...
    return diff_df
#For situations where src and tgt have different column names
def getDifferencesforExcel(dataframe1,dataframe2,primary_key1,primary_key2,ind=None):
    pkey1=primary_key1.split(',')
    pkey2=primary_key2.split(',')
    df_compare=dataframe1.merge(dataframe2,indicator=True,how="outer",left_on=pkey1,right_on=pkey2)
...
def compare(i,dataframe1,dataframe2,primary_key,strip_spaces=True,fill_na=True):
    ...
    #Striping the spaces for all data in the entire columns
...
def compareForExcel(dataframe1,dataframe2,primary_key1,primary_key2,strip_spaces=True,fill_na=True):
    mis_cnt=0
    mat_cnt=0
    pkey1=primary_key1.split(',')
    pkey2=primary_key2.split(',')

    df1= dataframe1.sort_values(by=pkey1) 
    df2= dataframe2.sort_values(by=pkey2)
